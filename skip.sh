...
echo -e "${CYAN}*-------------------*---------------------*${NC}"
echo -e "${YEL}* Check MDM - Skip enroll Auto for MacOS by *${NC}"
echo -e "${RED}*		     SKIP enroll.COM				*${NC}"
...
select opt in "${options[@]}"; do
	...
	"Autoypass on Recovery")
		...
		echo -e "Enter the username (Default: skipenroll) / Nhập tên User (Mặc định: skipenroll)"
		read realName
		realName="${realName:= skipenroll}"
		echo -e "${BLUE}Nhận username ${RED}WRITE WITHOUT SPACES / VIẾT LIỀN KHÔNG DẤU ${GRN} (Mặc định: skipenroll)"
		read username
		username="${username:=skipenroll}"
		echo -e "${BLUE}Enter the password (default: skipenroll1234) / Nhập mật khẩu (mặc định: skipenroll1234)"
		read passw
		passw="${passw:=skipenroll1234}"
